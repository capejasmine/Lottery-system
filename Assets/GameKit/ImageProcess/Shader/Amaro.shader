...
Shader "_Game/ImageProcess/Amaro" { 
		Properties{
			...
			_BlowoutTex("Blowout Texture",2D) = "Transparent" {}
			_Overlay("Overlay Texture",2D) = "Transparent" {}
			_AmaroMap("Amaro Texture",2D) = "Transparent" {}
		}
		...
		SubShader{
			...
			fixed4 frag(v2f i) : COLOR
			{
				...
				col = doAmaro(col,_BlowoutTex,_Overlay,_AmaroMap,i.texcoord);

				//float4 col = doAmaro(_MainTex,_BlowoutTex,_Overlay,_AmaroMap,i.texcoord);
