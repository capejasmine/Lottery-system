...
public class PainterProgressChecker : MonoBehaviour {
	...
	//拖文件到这上面
	public PainterCheckData checkData;
	...
	[Range(0.01f,1f)]
	public float editorBrushSize = 0.2f;
	#endif
	...
	public void SetDataByTexture(Texture2D texture,Texture penTexture,float brushScale,byte minAlpha = 10){
		...
		checkData = ScriptableObject.CreateInstance<PainterCheckData>();
	...
	public void Reset(){
		...
		if(m_painter && m_painter.penMat.mainTexture){
			float w = m_painter.penMat.mainTexture.width*m_painter.brushScale*0.005f;
			float h = m_painter.penMat.mainTexture.height*m_painter.brushScale*0.005f;
