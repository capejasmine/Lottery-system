...
public class Painter : MonoBehaviour {
    ...
    public void Drawing(Vector3 screenPosOrUV , Camera camera, RenderTexture rt,bool drawOutside=false,bool posIsUV = false){
		if(!m_inited) return;
		...
		{
            if (posIsUV)
                uvPos= SpriteHitPoint2UV(screenPosOrUV,posIsUV);
            else 
    ...
    public void ClickDraw(Vector3 screenPosOrUV , Camera camera=null , Texture pen=null, float penScale=1f , 
        Material drawMat = null , RenderTexture rt=null,bool posIsUV = false){
    ...
    Vector2 SpriteHitPoint2UV( Vector3 hitPointOrUV,bool isUV){
        Vector3 localPos = hitPointOrUV;
        if(!isUV) localPos = transform.InverseTransformPoint(hitPointOrUV);
        if (m_isUGUI)
        {
