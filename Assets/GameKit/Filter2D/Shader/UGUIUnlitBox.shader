...
Shader "_Game/Filter2D/UGUI Box Filter"
{
	Properties
	{
...
#ifdef SHARPNESS_ON
				fixed4 color = frag_boxblur(sharpening_matrix ,_MainTex,IN.texcoord,float2(_TexSizeX,_TexSizeY));
#else
				fixed4 color = frag_boxblur(gaussian_matrix ,_MainTex,IN.texcoord,float2(_TexSizeX,_TexSizeY));
#endif
