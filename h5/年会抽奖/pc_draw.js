//除chrome外，其他支持需要在服务器上运行才支持
    ...
    // var bgm = new Audio("img/pow1.mp3");
    // bgm.volume = 1;
    // bgm.play();
    ...
    if (local_handle.get("award_1")) {
        $('#award-01').show();
        var award1_storage =
         window.localStorage.getItem('award_1');
...
//		context.drawImage(back_img, 0, 0);
			...
			context.font = "bold 80px fantasy";
			//          if (name_zh.length <= 2) {
	...
	//		=name_zh.substring(0,1)+" "+name_zh.substring(1,2)+" "+name_zh.substring(2,3)+" "+name_zh.substring(3,4);
            if(name_str.length == 4) {
                context.fillText(name_str, 133, 385);  
            }else{
                context.fillText(name_str, 80, 385);
            }
...
//                  this.music.pause();
        ...
        $('.award').click(function () {
            if (isStart) {
                console.error('正在抽奖ing，不允许更改奖项设置');
                return false;
            }
